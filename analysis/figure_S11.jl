...
# Variable-strike contract: strike scales with next-period biomass rather
# than sitting at the constant midpoint of (b_target, b_limit). Matches the
# strike function used by src/run_variable_strike.jl so the solutions in
# results/variable_strike_*/sol.jld2 are consistent with this problem setup.
function strike(s,p)
...
# Risk-averse ("RA") panels across short / intermediate / long life
# histories under the variable-strike contract. Parameter files are
# unchanged versus figure_3; only the strike function and the solution
# path (results/variable_strike_*/sol.jld2) differ.
pars = "RA_short"
include("../parameters/$pars.jl")
X, Xmc = init_ranom_variables(p; Nquad = 15)
prob, El = init(action_grid, state_grid, δ, p, X, index, strike)
load_solution(prob, "results/variable_strike_$pars/sol.jld2")
Random.seed!(seed)
...
# Build a 3×3 grid: columns = parameter set (A, B, C); rows = biomass, USD, USD.
T = 100; t0 = 1

panels = [
    (label = "A", title = "Short life history",        x = x_A, s̄ = s̄_A),
    (label = "B", title = "Intermediate life history", x = x_B, s̄ = s̄_B),
    (label = "C", title = "Long life history",         x = x_C, s̄ = s̄_C),
]
...
Plots.savefig(fig, "manuscript/figure_S11.png")
fig
