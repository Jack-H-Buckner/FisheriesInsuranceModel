...
pars = "RA_intermidiate_higher_k_2"
include("../parameters/$pars.jl")
X, Xmc = init_ranom_variables(p; Nquad = 15)
prob, El = init(action_grid, state_grid, δ, p, X, index, strike)
load_solution(prob, "results/$pars/sol.jld2")
...
pars = "RA_intermidiate_higher_k_3"
include("../parameters/$pars.jl")
X, Xmc = init_ranom_variables(p; Nquad = 15)
prob, El = init(action_grid, state_grid, δ, p, X, index, strike)
load_solution(prob, "results/$pars/sol.jld2")
Random.seed!(seed)
x_D = simulation(prob, El, Xmc, T_sim; with_policy = false, s0 = s0)
s̄_D = p.s̄

# Build a 3×4 grid: columns = interest rate (A, B, C, D); rows = biomass, USD, USD.
T = 100; t0 = 1

panels = [
    (label = "A", title = "k = 0%", x = x_A, s̄ = s̄_A),
    (label = "B", title = "k = 2%", x = x_B, s̄ = s̄_B),
    (label = "C", title = "k = 4%", x = x_C, s̄ = s̄_C),
    (label = "D", title = "k = 6%", x = x_D, s̄ = s̄_D),
]
...
Plots.savefig(fig, "manuscript/figure_S7.png")
fig
