...
# Solve and simulate each parameter set sequentially at top level so that
# `p` from each parameter file is visible as a module global.
...
pars = "RA_intermidiate_lower_blimit"
include("../parameters/$pars.jl")
X, Xmc = init_ranom_variables(p; Nquad = 15)
prob, El = init(action_grid, state_grid, δ, p, X, index, strike)
load_solution(prob, "results/$pars/sol.jld2")
...
# Build a 3×2 grid: columns = b_limit case (A, B); rows = biomass, USD, USD.
T = 100; t0 = 1

panels = [
    (label = "A", title = L"B_{limit} = 0.2 B_0",  x = x_A, s̄ = s̄_A),
    (label = "B", title = L"B_{limit} = 0.02 B_0", x = x_B, s̄ = s̄_B),
]
...
for (i, panel) in enumerate(panels)
    ...
    is_left  = i == 1
    is_right = i == length(panels)
    # Legends live in a dedicated 3rd column (see legend stubs below) so the
    # data panels can stay equal width — none of the data subplots show one.
    legend_pos = :none
...
# Build per-row legend stub subplots so panels A and B can be exactly equal
# width. Each stub is an empty (framestyle = :none) plot whose only purpose
# is to render the legend for that row.
function legend_stub(entries)
    plt = Plots.plot(; framestyle = :none,
        xticks = false, yticks = false, grid = false,
        xlims = (0, 1), ylims = (0, 1), legend = :left,
        legend_foreground_color = :transparent)
    for (kind, color, label, kw) in entries
        if kind === :marker
            Plots.scatter!(plt, [NaN], [NaN]; color = color, label = label, kw...)
        else
            Plots.plot!(plt, [NaN, NaN], [NaN, NaN]; color = color, label = label, kw...)
        end
    end
    return plt
end

leg_row1 = legend_stub([
    (:marker, :black,                  "Stock",          ()),
    (:marker, :red,        string("High ", L"M_t"),      (alpha = 0.75,)),
    (:line,   :grey,                   "Threshold",      ()),
    (:line,   :grey,                   "Bankrupt",       (alpha = 0.25, linewidth = 8)),
])
leg_row2 = legend_stub([
    (:marker, :black, "Claims",   ()),
    (:marker, :grey,  "Income",   (alpha = 0.5,)),
    (:line,   :grey,  "Bankrupt", (alpha = 0.25, linewidth = 8)),
])
leg_row3 = legend_stub([
    (:marker, :black, "Savings",  ()),
    (:marker, :grey,  "Premiums", (alpha = 0.5,)),
    (:line,   :grey,  "Bankrupt", (alpha = 0.25, linewidth = 8)),
])

# 3-column layout: panel A | panel B | legend (column 3 is narrow).
layout = Plots.@layout [a b c{0.15w}; d e f{0.15w}; g h i{0.15w}]

fig = Plots.plot(row1..., leg_row1,
                 row2..., leg_row2,
                 row3..., leg_row3;
    layout = layout,
...
Plots.savefig(fig, "manuscript/figure_S9.png")
fig
